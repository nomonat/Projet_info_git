...
class Kmean(Traitement_image):
    ...
    def k_means(self, k: int = 4):
        ...
        centers = np.array([[195,229,235],[218,238,199],[255,255,249],[255,255,150]],dtype=np.uint8)
        km = KMeans(n_clusters=k, init=centers, n_init=1, random_state=0).fit(pixels)
        cols = np.array([[0,0,255],[34,139,34],[105,105,105],[255,215,0]])
    ...
    def build_reconstructed(self):
        ...
        def _fill(node,y0,x0,h,w):
            if isinstance(node, tuple):
                canvas[y0:y0+h, x0:x0+w] = node; return
            my, mx = h//2, w//2
